...
type Props = {
  post: Post & { user: User, community: Community, image: Image, _count: { likes: number, comments: number } }
}
...
<template>
  <UCard :ui="{base: 'cursor-pointer'}" @click="navigateTo(`/posts/${post.id}`)">
    ...
    <br>
    <div class="flex flex-col">
      <img v-if="post.image?.id" :src="`/api/images/${post.image.id}`" class="outlined-image" alt="Beispielbild">
    </div>
    <template #footer>
